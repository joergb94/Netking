                ...
                @for ($i = 5; $i < count($cardItems); $i += 4)
                        ...
                        <div class="row">
                                ...
                                <div class="col-12 theme1-col theme{{$data['themes_id']}}-padding" id="div-{{$cardItems[$i+1]['card_detail']->id}}">
                                        @include('Keypls.itemsUpdate.itemsKeypl.div'.$cardItems[$i+1]['item']->id,['ci' => $cardItems[$i+1],'template'=>'143','theme_shape'=>'theme1-shape'])
                                </div>
                                @endif
                                @if(isset($cardItems[$i+2]))
                                <div class="col-12 theme1-col theme{{$data['themes_id']}}-padding" id="div-{{$cardItems[$i+2]['card_detail']->id}}">
                                        @include('Keypls.itemsUpdate.itemsKeypl.div'.$cardItems[$i+2]['item']->id,['ci' => $cardItems[$i+2],'template'=>'143','theme_shape'=>'theme1-shape'])
                        
                                </div>
                                @endif
                                @if(isset($cardItems[$i+3]))
                                <div class="col-12 theme1-col theme{{$data['themes_id']}}-padding" id="div-{{$cardItems[$i+3]['card_detail']->id}}">
                                        @include('Keypls.itemsUpdate.itemsKeypl.div'.$cardItems[$i+3]['item']->id,['ci' => $cardItems[$i+3],'template'=>'143','theme_shape'=>'theme1-shape'])
